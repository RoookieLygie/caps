...
<main class="ttr-wrapper" style="background-color: #FBFBFB;">
   <?php include 'subh.php'; ?>
        ...
        <div class="row">
            <div class="col-lg-12 m-b30">
                ...
                <div class="heading-bx left">
                    <h2 class="m-b10 title-head">Payment <span>History</span></h2>
                </div>
                <section>
                    <style>
                        table {
        ...
        .action-buttons {
            display: flex;
            gap: 10px;
            border: 1px solid #ddd; /* Add a border to the action-buttons */
...
<body>
 ...
 $query = "SELECT * FROM reserve_booking2 WHERE email='$email' AND payments='paid'";
...
if($row)
            ?>
            <tr>
                <td><?php echo "Package" . " ".$a++; ?></td>
                
                <td><?php  if(($row['package'])==''){
                    echo 'Iterinary Booking';
                }else{
                    echo $row['package'];
                }
                    ?></td>
                ...
                <td class="action-buttons">
                    ...
                    <!-- <button class="view-button" onclick="viewAction(1)">View</button> -->
                    <!-- <a class="view-button" href='../../<?php echo $row['package'].".php"; ?>'>View</a> -->
                    <!-- <button class="delete-button" onclick="deleteAction(<?php //echo $row['carvid']; ?>)">Unlist</button> -->
                    <?php  $_SESSION['us']= $userId; ?>
                    <?php  echo $row['req_id'];  ?>
                    
              <!-- Add this script in the <head> section or at the end of the body -->
<script src="https://code.jquery.com/jquery-3.6.4.min.js"></script>
<script>
    function deleteAction(carvid) {
        if (confirm("Are you sure you want to unlist this item?")) {
            // Make an AJAX call to delete.php with the carvid parameter
            $.ajax({
                type: 'POST',
                url: 'delete.php',
                data: { carvid: carvid },
                ...
                <td>
               <?php  echo '<a style="font-size:12px;" class="paid-status" href="receipt.php?='.uniqid().uniqid().uniqid().uniqid().rand(111111111,99999999).'&alt='.uniqid().uniqid().'&id=' . $row["id"] .'&'.uniqid().uniqid(). '">Receipt</a>';?>
                    <!-- <a href="receipt.php?id=<?php  echo $row['id']; ?>">Receipt</a> -->
                </td>
            </tr>
           <?php    }
 } ?>
    ...
    <script>
        function viewAction(rowNumber) {
            alert('View action for row ' + rowNumber);
            // Add your view action logic here
        }

        // function deleteAction(rowNumber) {
