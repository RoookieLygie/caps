...
<?php include 'subh2.php'; ?>
        ...
        <body>
          ...
          if(isset($_POST['submit'])){
            ...
            $update = mysqli_query($db, "UPDATE registration SET fullname='$name', password='$pass', email='$email' WHERE id='$id'");
        ...
        ?>
               ...
               <form method="POST" >
                <?php 
                $id=$_GET['id'];
$select=mysqli_query($db,"SELECT * FROM registration WHERE id='$id'");
$row=mysqli_fetch_array($select);

?>
                <!-- <input type="file" accept="image/*" id="image-input" onchange="previewImage()"> -->
                <label for="">User Name</label>
                <input type="text" name='username' id="name-input" value="<?php echo $row['fullname']; ?>">
                <label for="">Password</label>
                <input type="text" name='password' id="password-input" value="<?php echo $row['password']; ?>">
                <label for="">Email</label>
                <input type="email" name='email' id="password-input" value="<?php echo $row['email']; ?>">
                
                <input type="submit" name="submit" id="save-button" value="Update">
